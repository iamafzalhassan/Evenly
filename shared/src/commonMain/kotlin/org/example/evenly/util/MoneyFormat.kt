...
object MoneyFormat {
    ...
    fun format(money: Money): String {
        ...
        val whole = DecimalInput.groupThousands((magnitude / currency.minorScale).toString())
        val fraction = (magnitude % currency.minorScale).toString().padStart(currency.minorDigits, '0')
        return if (currency.minorDigits == 0) "$sign${currency.name} $whole" else "$sign${currency.name} $whole${DecimalInput.DECIMAL_SEPARATOR}$fraction"
    }
